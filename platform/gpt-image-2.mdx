...
任何支持 OpenAI 协议的客户端都能直接用 —— 接口地址填 `https://ruoli.dev/v1`，模型名填 `gpt-image-2`。常见客户端的入口：

| 客户端 | 入口 |
|------|------|
| Lobe Chat | 设置 → OpenAI → 开启绘图，模型选 `gpt-image-2` |
| NextChat | 自定义端点 → 启用 DALL-E，模型 `gpt-image-2` |
| [Cherry Studio](/tools/cherry-studio) | 自定义提供商 → 图像生成模式选 `gpt-image-2` |
| SillyTavern | Image Generation → OpenAI 兼容 → 端点 `https://ruoli.dev/v1` |
