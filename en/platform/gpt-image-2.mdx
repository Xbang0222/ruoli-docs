...
Any OpenAI-compatible client works — set the endpoint to `https://ruoli.dev/v1` and the model to `gpt-image-2`. Common client entry points:

| Client | Entry |
|--------|-------|
| Lobe Chat | Settings → OpenAI → enable image generation, model `gpt-image-2` |
| NextChat | Custom endpoint → enable DALL-E, model `gpt-image-2` |
| [Cherry Studio](/en/tools/cherry-studio) | Custom provider → image-generation mode, model `gpt-image-2` |
| SillyTavern | Image Generation → OpenAI-compatible → endpoint `https://ruoli.dev/v1` |
| n8n / Dify | HTTP Request node hits `/v1/images/generations` directly |
